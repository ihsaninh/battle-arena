 ...
 *   2. Run tests: bun test __tests__/api/battle/join-room.test.ts (in terminal 2)
 */
...
import { createRoomViaAPI, joinRoomViaAPI } from '../../helpers/api-helpers';
...
describe('POST /api/battle/rooms/[roomId]/join', () => {
  ...
  beforeAll(async () => {
    ...
    if (!running) {
      ...
      console.error(
        '   Terminal 2: bun test __tests__/api/battle/join-room.test.ts\n'
      );
