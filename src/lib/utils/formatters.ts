...
export const getDifficultyLabel = (
  ...
  if (typeof difficulty === 'string') {
    const normalized =
      difficulty.toLowerCase() as keyof typeof DIFFICULTY_LABEL_MAP;
  ...
  if (difficulty === 1) {
    return lang === 'id'
      ? DIFFICULTY_LABEL_MAP.easy.id
      : DIFFICULTY_LABEL_MAP.easy.en;
  }
  if (difficulty === 3) {
    return lang === 'id'
      ? DIFFICULTY_LABEL_MAP.hard.id
      : DIFFICULTY_LABEL_MAP.hard.en;
  }
  return lang === 'id'
    ? DIFFICULTY_LABEL_MAP.medium.id
    : DIFFICULTY_LABEL_MAP.medium.en;
};
